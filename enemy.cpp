...
enemy::enemy(const float x, const float y) : tank(x, y, "Textures/enemyset.png"), timeBeforeMoving(0), timeBeforeShot(0)
															//определение конструктора - используем унаследованный конструктор, 
															//передаем в него начальный координаты танка-врага и файл с его спрайтом,
															//устанавливаем время до выстрела и время до смены направления движения по умолчанию на 0
{
	t_Sprite.setTextureRect(sf::IntRect(0 + 64 * (int)t_CurrentFrame, 2 * 64, 64, 64)); //обрезаем спрайт танка-врага по той 'строчке', в которой он движется вниз 
...
void enemy::move(const sf::Int64 time) //определение функции-перемещения врага
{
	...
	if (timeBeforeMoving > 1000 + rand() % 501) //если танк-враг движется от 501 до 1000 мс
	{
		t_dir = TDIR(rand() % 3); //случайно меняем его направление движения
		timeBeforeMoving = 0; //устанавливаем время до смены движения на ноль
	}

	switch (t_dir) //если танк-враг перемещается:
	{
